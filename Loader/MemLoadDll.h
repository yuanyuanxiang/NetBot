    ...
    for (i=0; i<module->headers->FileHeader.NumberOfSections; i++, section++) {
        ...
        if (size > 0) {
            ...
            if (VirtualProtect((LPVOID)section->Misc.PhysicalAddress, section->SizeOfRawData, protect, &oldProtect) == 0) {
                Mprintf("Error protecting memory page\n");
            }
        }
    }
}
...
HMEMORYMODULE MemoryLoadLibrary(const void *data)
{
    ...
    if (dos_header->e_magic != IMAGE_DOS_SIGNATURE) {
        Mprintf(_T("Not a valid executable file.\n"));
    ...
    if (code == NULL) {
        Mprintf(_T("Can't reserve memory\n"));
    ...
    if (result->headers->OptionalHeader.AddressOfEntryPoint != 0) {
        ...
        if (DllEntry == 0) {
            Mprintf(_T("Library has no entry point.\n"));
        ...
        if (!successfull) {
            Mprintf(_T("Can't attach library.\n"));
